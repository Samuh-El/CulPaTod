<title>Pago Paypal</title>
	...
	if(!empty($_GET['paymentID']) && !empty($_GET['payerID']) && !empty($_GET['token']) && !empty($_GET['pid']) )
	{
		...
		$billingToken= $_REQUEST['billingToken'];
		$intent= $_REQUEST['intent'];
		$objetoPaypal = $_REQUEST['objetoPaypal'];
		...
		<?php	
			...
			$sql ="INSERT INTO transaccion (runComprador,codigoTransaccion,fechaTransaccion,espectaculo_idEspectaculo,notification_token,receiver_id,bank,payer_name,payer_email,responsible_user_email,payment_method,valorTransaccion,orderID,billingToken,intent,objetoPaypal) 
			VALUES (
				'paypal','".$paymentID."',CURRENT_TIMESTAMP,'".$idEspectaculo."','".$token."',0,'0','0','".$email."','0','0','".$valorTransaccion."','".$orderID."','".$billingToken."','".$intent."','".$objetoPaypal."'
			);";
