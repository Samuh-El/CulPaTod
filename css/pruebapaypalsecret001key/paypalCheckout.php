  ...
  onAuthorize: function (data, actions) {
	return actions.payment.execute()
	  .then(function () { 
		...
		let billingToken = data['billingToken'];
		let intent = data['intent'];
		let objetoPaypal = JSON.stringify(data);

		alert(objetoPaypal);

		window.location = "<?php echo PayPalBaseUrl ?>orderDetails.php?paymentID="+
		data.paymentID+
		"&nombreProducto="+document.getElementById("4").value+
		"&valorTransaccion="+document.getElementById("3").value+
		"&email="+document.getElementById("1").value+
		"&payerID="+data.payerID+
		"&objetoPaypal="+objetoPaypal+ // todo el objeto
		"&orderID="+data.orderID+
		"&billingToken="+data.billingToken+
		"&intent="+data.intent+
		"&idEspectaculo="+document.getElementById("2").value+
